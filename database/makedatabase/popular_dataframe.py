...
racas = [
    ...
    'Half-orc da cidade',
    'Half-orc da selva',
    'Halfling das planícies',
    'Halfling das montanhas',
    'Halfling das florestas',
...
def popular_dataframe(df):
    ...
    df['Nome'] = [
        '{0}'.format(nome)
        for nome in np.random.choice(nomes, size=1500, replace=True)
    ]
    ...
    df['Classe'] = np.random.choice(classes, size=1500, replace=True)

    # Gerar raças aleatórias
    df['Raça'] = np.random.choice(racas, size=1500, replace=True)
